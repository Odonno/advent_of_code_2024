...
pub type DistanceMap =
  Dict(common.Path, Int)

pub type NextNeighbors =
  Dict(common.Path, List(common.Path))
...
pub fn main(input: String, use_sample: Bool) -> Nil {
  ...
  let initial_path =
    common.Path(position: start_position, direction: initial_direction)
  ...
  let all_valid_paths =
    get_valid_paths(next_neihbors, initial_path, end_position)

  let result =
    all_valid_paths
    |> list.flat_map(fn(p) { p |> list.map(fn(p) { p.position }) })
    |> set.from_list
    |> set.size
...
fn create_distance_map(
  ...
  start_path: common.Path,
  max_score: Int,
) -> DistanceMap {
  create_distance_map_inner(
    [#(start_path, 0)] |> dict.from_list,
    map,
    start_path,
    max_score,
    0,
...
fn create_next_neihbors_map(distance_map: DistanceMap) -> NextNeighbors {
  ...
  |> dict.map_values(fn(path, score) {
    ...
    |> dict.filter(fn(key, _) { is_neighbor(key.position, path.position) })
    |> dict.keys
  })
}
...
fn is_neighbor(
  ...
  other_position: common.Position,
) -> Bool {
  let a =
    position.0 == other_position.0
    && {
      position.1 == other_position.1 - 1 || position.1 == other_position.1 + 1
    }
  let b =
    position.1 == other_position.1
    && {
      position.0 == other_position.0 - 1 || position.0 == other_position.0 + 1
    }
  ...
  case target == current_position {
    ...
    _ -> {
      ...
      |> list.flat_map(fn(next_path) {
        get_valid_paths_inner(next_neihbors, next_path, target, [
          next_path,
          ..acc
        ])
      })
