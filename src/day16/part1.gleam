  ...
  let shortest_path =
    find_shortest_path(
      ...
      dict.new()
        |> dict.insert(
          initial_path,
          calculate_path_distance(initial_path, end_position),
        ),
      dict.new(),
      [initial_path],
    )
...
fn find_shortest_path(
  ...
  case current_path.position == target {
    ...
    False -> {
      ...
      let #(g_scores, f_scores, parents, next_paths) =
        neighbors_with_score
        |> list.fold(
          #(g_scores, f_scores, parents, []),
          fn(acc, neighbor_with_score) {
      ...
      find_shortest_path(
        ...
        parents,
        visitable_paths
          |> list.filter(fn(x) { x != current_path })
          |> list.append(next_paths),
      )
    }
  }
}
