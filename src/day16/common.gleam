...
pub type ReeinderMaze =
  Dict(Position, MazeCell)
...
pub fn get_neighbors(maze: ReeinderMaze, path: Path) -> List(#(Path, Int)) {
  ...
  let turn_directions_with_score =
    turn_directions
    |> list.map(fn(x) { #(x, turn_cost) })

  let directions_with_score = [
    forward_direction_with_score,
    ..turn_directions_with_score
  ]
  ...
  |> list.flat_map(fn(x) {
    ...
    case next_cell {
      Empty -> {
        let next_path = #(
          Path(position: next_cell_position, direction: direction),
          score,
        )
        [next_path]
      }
