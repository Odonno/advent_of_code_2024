...
fn regenerate_run_function() {
  ...
  let puzzle_folders =
    puzzle_folders |> list.filter(fn(f) { f |> string.starts_with("day") })

  io.debug(puzzle_folders)

  let tree = string_tree.new()

  let append_newline = fn(tree) { tree |> string_tree.append("\n") }
...
",
  ...
  let tree =
    ...
    |> list.fold(tree, fn(tree, folder) {
      ...
      tree
      |> string_tree.append(
        "import " <> part1_module <> " as day" <> day <> "_part1",
      )
      |> append_newline
      |> string_tree.append(
        "import " <> part2_module <> " as day" <> day <> "_part2",
      )
      |> append_newline
    })
