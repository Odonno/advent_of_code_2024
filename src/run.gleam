...
fn display_execution_time(before, after) {
  ...
  let max_seconds = 28_944_000
  // hack for unkown reason
  ...
  case total_seconds {
    seconds if seconds > 3 ->
      io.println(
        "Took " <> seconds |> int.to_string <> " seconds. Really slow...",
      )
    seconds if seconds > 0 ->
      io.println("Took " <> seconds |> int.to_string <> "s. A bit slow, right?")
