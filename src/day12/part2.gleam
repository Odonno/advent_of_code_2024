...
fn calculate_perimeter(plant: common.Plant, region_positions: Set(common.Position), map: common.GardenMap) -> Int {
    let min_x = 0
    let assert Ok(max_x) = region_positions |> set.map(fn(position) { position.0 }) |> set.to_list |> list.sort(int.compare) |> list.last

    let min_y = 0
    let assert Ok(max_y) = region_positions |> set.map(fn(position) { position.1 }) |> set.to_list |> list.sort(int.compare) |> list.last

    let lines_0 = iterator.range(min_y, max_y)
        |> iterator.fold(#(0, [] |> set.from_list), fn(acc, y) {
            let #(total, previous_row) = acc

            let current_row = iterator.range(min_x, max_x)
                |> iterator.filter(fn(x) {
                    region_positions |> set.contains(#(x, y))
                })
                |> iterator.filter(fn(x) {
    ...
    let lines_1 = iterator.range(min_y, max_y)
        |> iterator.fold(#(0, [] |> set.from_list), fn(acc, y) {
            let #(total, previous_row) = acc
    ...
    let lines_2 = iterator.range(min_x, max_x)
        |> iterator.fold(#(0, [] |> set.from_list), fn(acc, x) {
            let #(total, previous_row) = acc

            let current_row = iterator.range(min_y, max_y)
    ...
    let lines_3 = iterator.range(min_x, max_x)
        |> iterator.fold(#(0, [] |> set.from_list), fn(acc, x) {
            let #(total, previous_row) = acc
