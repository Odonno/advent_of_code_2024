...
type PriceChangeIndexes = Dict(PriceChange, Set(Int))
...
pub fn main(_: String, use_sample: Bool) -> Nil {
    ...
    let prices_per_row = values
        |> list.map(fn(value) {
            let secrets =
                iterator.range(1, iterations)
                |> iterator.scan(value, fn(value, _) {
                    common.iterate(value)
                })
    ...
    let possible_sequences = price_changes_per_row
    ...
    let price_change_indexes_list: List(PriceChangeIndexes) =
        ...
        |> list.map(fn(p) {
            p |> list.index_fold(dict.new(), fn(acc, price_change, index) {
                acc
