...
pub fn main(input: String, use_sample: Bool) -> Nil {
    ...
    let result = designs
        |> list.filter(fn(design) {
            is_design_possible(patterns, design)
        })
        |> list.length
...
fn is_design_possible(patterns: common.TowelPatterns, design: common.TowelDesign) -> Bool {
    patterns
    |> list.any(fn(pattern) {
        let first = design |> string.slice(0, pattern |> string.length)
        let rest = design |> string.slice(pattern |> string.length, design |> string.length)
