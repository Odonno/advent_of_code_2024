...
pub fn main(_: String, use_sample: Bool) -> Nil {
    ...
    let #(a_value, _) = iterator.single(0)
        |> iterator.cycle
        |> iterator.fold_until(#(0, length - 1), fn(acc, _) {
            let #(a_value, index) = acc
            let registers = dict.new() |> dict.insert("A", a_value) |> dict.insert("B", 0) |> dict.insert("C", 0)
            let output = common.execute(program, registers)

            let to_compare = length - index

            let is_correct = 
                program |> list.reverse |> list.take(to_compare) == output |> list.reverse |> list.take(to_compare) 
            ...
            case is_correct {
                ...
                False -> {
                    let adder = int.power(8, index |> int.to_float) |> result.unwrap(0.0) |> float.truncate
    ...
    common.expect(use_sample, result, 117440)
}