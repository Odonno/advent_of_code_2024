...
pub fn main(input: String, use_sample: Bool) -> Nil {
    ...
    common.expect(use_sample, result, 11387)
}
    ...
    case remaining {
        ...
        [first, ..rest] -> {
            ...
            let assert Ok(concat_multiplier) = 
                10 |> int.power(number_of_digits |> int.to_float) |> result.map(float.truncate)

            let concat_total = value * concat_multiplier + first

            is_valid_inner(expected, addition_total, rest) || is_valid_inner(expected, multiplication_total, rest) || is_valid_inner(expected, concat_total, rest)
        }
    }
}