...
pub type Swaps = Dict(common.Wire, common.Wire)
...
pub fn main(_: String, use_sample: Bool) -> Nil {
    ...
    let assert Ok(max_bit_index) = 
        ...
        |> list.map(fn(k) { k |> string.slice(1, 2) |> int.parse |> result.unwrap(0) })
        |> list.sort(int.compare)
        |> list.last

    let swapped_wires = generate_swaps(gates, dict.new(), max_bit_index)

    let result = swapped_wires |> dict.keys |> list.sort(string.compare) |> string.join(",")

    common.expect_str(use_sample, result, "z00,z01,z02,z05")
}

fn generate_swaps(gates: common.Gates, swaps: Swaps, max_bit_index: Int) -> Swaps {
    let current_output = get_swapped_output(gates, swaps, "x00", "y00", common.And)
...
fn generate_swaps_inner(
    ...
    case index > max_bit_index {
        True -> swaps
        False -> {
            let current_wire_value = index |> int.to_string |> string.pad_start(2, "0")

            let x_wire = "x" <> current_wire_value
            let y_wire = "y" <> current_wire_value
            let z_wire = "z" <> current_wire_value

            let assert Some(xor_output) = get_swapped_output(gates, swaps, x_wire, y_wire, common.Xor)
            let assert Some(and_output) = get_swapped_output(gates, swaps, x_wire, y_wire, common.And)

            let current_xor_output = case current_output {
                Some(current_output) -> get_swapped_output(gates, swaps, current_output, xor_output, common.Xor)
                _ -> None
            }
            let current_and_output = case current_output {
                Some(current_output) -> get_swapped_output(gates, swaps, current_output, xor_output, common.And)
                _ -> None
            }
            ...
            case current_xor_output, current_and_output {
                None, None -> {
                    ...
                    generate_swaps_inner(gates, swaps, max_bit_index, index, current_output)
                }
                ...
                _, _ -> {
                    ...
                    let current_output = get_swapped_output(gates, swaps, and_output, current_and_output, common.Or)

                    generate_swaps_inner(gates, swaps, max_bit_index, index + 1, current_output)
                }
            }
        }
    }
