                ...
                common.Up | common.Down -> {
                    ...
                    case next_cell {
                        ...
                        common.LeftBox | common.RightBox -> {
                            let #(can_push, box_positions) = get_pushable_boxes(
                                map, 
                            ...
                            case can_push {
                                True -> {
                                    ...
                                    let map = box_positions
                                        |> set.to_list
                                        |> list.fold(map, fn(map, p) {
                                            map |> dict.delete(p)
                                        })
                ...
                common.Left -> {
                    let next_wall_or_empty_items = 
                        ...
                        |> dict.filter(fn(pos, cell) {
                            { cell == common.Wall || cell == common.Empty } && pos.0 < robot_position.0 && pos.1 == robot_position.1
                        })
                    
                    let closest_item = next_wall_or_empty_items
                        |> dict.to_list
                        |> list.sort(fn(a, b) { 
                            case a.0.0 > b.0.0 {
                                True -> Gt
                        ...
                        Ok(#(pos, common.Empty)) -> {
                            let map = 
                                ...
                                |> iterator.fold(map, fn(map, x) {
                                    ...
                                    let assert Ok(previous_cell) = map |> dict.get(#(previous_x, pos.1))
...
fn get_two_boxes_positions(map: common.WarehouseMap, position: common.Position) -> List(common.Position) {
    ...
    case box_positions {
        [first, ..rest] -> {
            ...
            let #(can_push, next_boxes) = case next_cell {
                ...
                common.LeftBox | common.RightBox -> {
                    get_pushable_boxes(map, get_two_boxes_positions(map, next_cell_position), direction)
                }
                _ -> panic as { "Not implemented" }
            }

            let #(rest_can_push, rest_positions) = get_pushable_boxes(map, rest, direction)
