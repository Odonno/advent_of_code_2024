  ...
  let iterations =
    ...
    |> iterator.fold(iterator.empty(), fn(acc, d) {
      let from_first_iterator =
        iterator.repeat(FromFirst) |> iterator.take(d.length)
      let from_last_iterator =
        iterator.repeat(FromLast) |> iterator.take(d.free_space)

      acc
      |> iterator.append(from_first_iterator)
      |> iterator.append(from_last_iterator)
    })
