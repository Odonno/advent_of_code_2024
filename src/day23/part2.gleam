...
pub fn main(_: String, use_sample: Bool) -> Nil {
  ...
  let larger_link = get_unique_connection(current_two_connections, links)
...
fn get_unique_connection(
  current_connections: Connections,
  links: common.NetworkLinks,
) -> Connection {
...
fn get_computers_with_n_connections(
  current_connections: Connections,
  links: common.NetworkLinks,
) -> Connections {
  current_connections
  |> list.flat_map(fn(n) {
    let all_links =
      n |> list.map(fn(l) { links |> dict.get(l) |> result.unwrap(set.new()) })
    ...
    |> set.to_list
    |> list.map(fn(c) { [c, ..n] |> list.sort(string.compare) })
  })
  |> set.from_list
  |> set.to_list
}
