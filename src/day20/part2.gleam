...
pub fn main(input: String, use_sample: Bool) -> Nil {
  ...
  let assert Ok(max_distance) =
    distance_map |> dict.values |> list.sort(int.compare) |> list.last
  ...
  let possible_cheats =
    ...
    |> iterator.flat_map(fn(start) {
      let start_positions =
        distance_map
        |> dict.filter(fn(_, value) { value == start })
        |> dict.keys

      let possible_end_positions =
        distance_map
        |> dict.filter(fn(_, value) { value > { start + min_diff } })
        |> dict.keys
...
fn create_distance_map(
  map: common.Map,
  start_position: common.Position,
) -> DistanceMap {
  create_distance_map_inner(
    [#(start_position, 0)] |> dict.from_list,
    map,
    start_position,
    0,
  )
