...
pub type Cheat = common.Position

pub fn main(input: String, use_sample: Bool) -> Nil {
    let #(map, start_position, end_position) = common.parse(input)
    
    let current_course_time = find_shortest_time(
        map,
        end_position,
        dict.new() |> dict.insert(start_position, 0),
        dict.new() |> dict.insert(start_position, common.calculate_distance(start_position, end_position)),
        dict.new(),
        [start_position],
        None
    )

    let assert Ok(max_x) = map |> dict.keys |> list.map(fn(x) { x.0 }) |> list.sort(int.compare) |> list.last
    let assert Ok(max_y) = map |> dict.keys |> list.map(fn(x) { x.1 }) |> list.sort(int.compare) |> list.last
    ...
    let saved_times = 
        ...
        |> iterator.map(fn(cheat) {
            let cheat_course_time = find_shortest_time(
                map,
                end_position,
                dict.new() |> dict.insert(start_position, 0),
                dict.new() |> dict.insert(start_position, common.calculate_distance(start_position, end_position)),
...
fn calculate_shortest_time(parents: common.ParentMap, current: common.Position, cheat_opt: Option(Cheat)) -> Int {
...
fn get_neighbors(map: common.Map, position: common.Position, cheat_opt: Option(Cheat)) -> List(common.Position) {
